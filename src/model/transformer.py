...
class TransformerForNER(nn.Module):
    def __init__(self, base_transformer, hidden_size, num_labels):
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask):
        enc_src = self.transformer.enc_embedding(input_ids)
        for layer in self.transformer.encoder_layers:
            enc_src = layer(enc_src, self.transformer.make_src_mask(input_ids))

        sequence_output = self.dropout(enc_src)
